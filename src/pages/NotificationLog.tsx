    ...
    <AppLayout>
      <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-foreground">WhatsApp Notification Log</h1>
            <p className="text-sm text-muted-foreground mt-1">Riwayat pengiriman notifikasi WhatsApp via Fonnte</p>
          </div>
          ...
          <CardContent className="p-4">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder="Cari user, nomor, atau pesan..."
                  value={search}
                  ...
                  filteredLogs.map((log: any) => (
                    <TableRow key={log.id}>
                      <TableCell className="font-medium">
                        {profileMap.get(log.user_id) || "Test User"}
                      </TableCell>
                      ...
                      <TableCell className="text-xs text-muted-foreground whitespace-nowrap">
                        {log.sent_at
                          ? format(new Date(log.sent_at), "dd MMM yyyy HH:mm", { locale: idLocale })
                          : "-"}
                      </TableCell>
                    </TableRow>
                  ))
